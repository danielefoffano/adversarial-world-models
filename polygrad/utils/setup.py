...
class Parser(Tap):
    ...
    def parse_args_no_cli(self, experiment=None):
        args = super().parse_args(known_only=True)
        if not hasattr(args, 'config'): return args
        args = self.read_config(args, experiment)
        self.eval_fstrings(args)
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'env_name' in dir(args) and 'exp_name' in dir(args):
            args.savepath = os.path.join(args.logbase, args.env_name, args.exp_name)
